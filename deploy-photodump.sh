...
until $(curl --output /dev/null --silent --head --fail http://$NEW_DROPLET_IP:8080/swagger-ui.html); do
    ...
    attempts=$(($attempts+1))
    echo "Waiting for app to respond at http://$NEW_DROPLET_IP:8080/swagger-ui.html ..."
    sleep 10
done
